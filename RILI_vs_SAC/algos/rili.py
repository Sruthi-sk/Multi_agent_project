...
class RILI(object):
    ...
    def update_parameters(self, memory, batch_size):
        ...
        Sample batch_size of consecutive interactions tau1, tau2, tau3, tau4, tau5
        States and rewards for first interaction - get from tau1 (shape of batchsize) same for second interaction, third , fourth 
        States,action, rewa`    rds, nextstate,done for fifth interaction from tau5
        push all to device as torch float tensors
        making all tau only contain state and reward and making them in shape (batchsize, (sequence_length * (state_dimension + 1))) 
          because in NN  each input must be a one-dimensional vector.
        train the autoencoder
        get latents for current and next from autoencoder using tau
        latent_plus(z_plus, z_plus_std, p_plus) latent_curr(z_curr, z_curr_std, p_curr) make it repeat for timestep times
        state_batch and next_state_batch  = reshape states5 and next_states5 as (-1, self.state_dim))
        state_batch concat with latent_curr and next_state_batch= concat(next_state_batch, latent_next)
        action_batch reshape as (-1, self.action_dim) and reward_batch reshaped from rewards5, (-1, 1)
        """

        # Sample a batch from memory
        # # Sample batch of consecutive interactions tau1, tau2, tau3, tau4, tau5
        tau1, tau2, tau3, tau4, tau5 = memory.sample(batch_size=batch_size)

        # States and rewards for first interaction
        #- get from tau1 (shape of batchsize) same for second interaction, third , fourth 
