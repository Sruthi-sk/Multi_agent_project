...
# class RILI_Autoencoder(nn.Module):
    # state_dim = dim of observation space , reward_dim=1, latent_dim=10, hidden_dim=64,
    # def __init__(self, state_dim, reward_dim, latent_dim, hidden_dim, timesteps=10):
        # Strategy encoder (tau^i --> z^i)  (state_dim+reward_dim)*timesteps -> hidden_dim -> latent_dim (enc_zcurr_1,2,3)
        # Partner encoder (context latent_dim*4 z1, z2, z3, z4 --> p, pmean and pvariance) used in encoder_partner pmean and var size is latent_dim
        # Dynamics (z^i, p^i --> z^i+1 of shape hiddendim and its mean and sd both of shape latentdim)  latent_dim*2 ->hidden_dim-> latent_dim
        # Decoder architecture (states, z (shape obs_space_dim*timesteps)--> rewards (shape reward_dim*timesteps))
    # self.apply(weights_init_)
    # def encoder_strategy(self, tau): (tau^i --> z^i) 
    # def encoder_partner(self, z1, z2, z3, z4):   # calculates dynamics using enc_p  #     return x, x_mean, x_std
    # def dynamics(self, z, p):    #     # calculates znext    #     return z, z_mean, z_std
    # def encoder(self, tau1, tau2, tau3, tau4): 
        # z1 = self.encoder_strategy(tau1) # repeat for z2,z3,z4
        # p, p_mean, p_std = self.encoder_partner(z1, z2, z3, z4)
        # z5, z5_mean, z5_std = self.dynamics(z4, p)
        # return z5, z5_mean, z5_std, p, p_mean, p_std
    # def decoder(self, context): # to predict rewars
        # return (self.dec3(x) - 1.0) * 100
    
    # def forward(self, tau1, tau2, tau3, tau4, states1, rewards1, states2, rewards2, states3, rewards3, states4, rewards4, states5, rewards5):
        # z1str = self.encoder_strategy(tau1) # same for tau 2,3, 4
        # p, p_mean, p_std = self.encoder_partner(z1str, z2str, z3str, z4str)
        # z2hat, z2_mean, z2_std = self.dynamics(z1str, p) # same till z4str
        # s1 = torch.flatten(states1, 1) #till s5
        # r1str = self.decoder(torch.cat((s1, z1str), 1)) till r4str
        # r2hat = self.decoder(torch.cat((s2, z2hat), 1)) till r5hat
        # l1str = self.loss_fcn(r1str, rewards1) # till l4str
        # l2hat = self.loss_fcn(r2hat, rewards2) # till l5hat
        # kl_2hat = -0.5 * (1 + torch.log(z2_std.pow(2)) - z2_mean.pow(2) - z2_std.pow(2)).mean() till k5hat
        # kl_p = -0.5 * (1 + torch.log(p_std.pow(2)) - p_mean.pow(2) - p_std.pow(2)).mean()
        # curr_loss = l1str+l2str+l3str+l4str
        # next_loss = l2hat+l3hat+l4hat+l5hat
        # ae_loss = curr_loss + next_loss
